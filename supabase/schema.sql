...
create index if not exists idx_messages_recipient_time on public.messages(recipient_id, created_at desc);

alter table public.messages enable row level security;

create policy if not exists "User read own conversations" on public.messages
for select using (auth.uid() = sender_id or auth.uid() = recipient_id);

create policy if not exists "User insert own messages" on public.messages
for insert with check (auth.uid() = sender_id);

create policy if not exists "User update own message read flag" on public.messages
for update using (auth.uid() = recipient_id) with check (true);
